...
- **`shamir/`** — the optimized: one shared Shamir/Straus doubling chain (508 → 254 doublings), elliptic-curve ops as reusable multi-return functions (`OP_DEFINE`/`OP_INVOKE`), verified-inverse-on-stack, and each chunk **loops over its bit-range** (the loop body is compiled once, so op-cost — not size — binds), with per-chunk tuned padding. Carries a 5-coord state (R + the runtime public inputs). **2 chunks, 13,266 script bytes, ~8.56M op-cost.**
