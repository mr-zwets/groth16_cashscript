...
// ---------- 3. THE CLAIM: A/C cofactor components leave the accepted set unchanged ----------
...
// maul A (pair 0 holds (-A, B)): -A' = -A + T  <=>  A' = A - T (cofactor junk on A)
const maulPair = (idx, delta) => pairs.map((pr, i) => (i === idx ? { ...pr, P: pr.P.add(delta) } : pr));
const gA = millerBatchOps(maulPair(0, T)).boundary;
...
// maul C (pair 3 holds (C, delta))
const gC = millerBatchOps(maulPair(3, T)).boundary;
check('C + cofactor-torsion: g^h == 1 still (accepts)', isOne(powF(gC, h)));

// NEGATIVE control: change the r-part of A by a subgroup point => must be rejected
const gBad = millerBatchOps(maulPair(0, G1.BASE)).boundary;
...
// NEGATIVE control on C too
const gBadC = millerBatchOps(maulPair(3, G1.BASE)).boundary;
